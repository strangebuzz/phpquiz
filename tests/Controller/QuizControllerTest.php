...
final class QuizControllerTest extends WebTestCase
{
    ...
    public function testQuestionSubmit(): void
    {
        ...
        self::assertStringContainsString(sprintf('Question 1/%d', $questionCount), (string) $client->getResponse()->getContent());
        ...
        foreach (range(1, $questionCount) as $questionRank) {
            ...
            if ($questionRank === $questionCount) {
                $client->followRedirect();
                self::assertStringContainsString('Your score:', (string) $client->getResponse()->getContent());
            } else {
                self::assertStringContainsString(sprintf('Question %d/%d', $questionRank + 1, $questionCount), (string) $client->getResponse()->getContent());
            }
        }
    }
}
